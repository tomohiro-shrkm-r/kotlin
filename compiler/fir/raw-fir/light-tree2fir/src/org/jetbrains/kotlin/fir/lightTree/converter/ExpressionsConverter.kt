        ...
        dotQualifiedExpression.forEachChildren {
            when (it.tokenType) {
                ...
                SAFE_ACCESS -> {
                    ...
                    val firExpression =
                        getAsFirExpression<FirExpression>(it, "Incorrect ${if (isSelector) "selector" else "receiver"} expression")
                    if (isSelector && it.tokenType != TokenType.ERROR_ELEMENT) {
                        if (it.tokenType is KtNameReferenceExpressionElementType || it.tokenType == KtNodeTypes.CALL_EXPRESSION) {
                            firSelector = firExpression
                        } else {
                            val errorExpression = buildErrorExpression {
                                source = it.toFirSourceElement()
                                diagnostic = ConeSimpleDiagnostic("The expression cannot be a selector (occur after a dot)", DiagnosticKind.IllegalSelector)
                            }
                            errorExpression.replaceExpression(firExpression)
                            firSelector = errorExpression
                        }
                    } else {
